...
vk.setOptions({ token: 'a0ce93d39d4ce046e2671eaca1eff06d2b4046097a9e7fcbea1b300e5122f0d653dd1d30e8a6904d940e9', pollingGroupId:166217385 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[club18915434\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club182915434\|(.*)\]/ig, '').trim();

	if(!users.find(x=> x.id === message.senderId))
	{
		message.send (`[👋🏻]Привет,я игровой бот "Chappy".
...
cmd.on(/^(?:eval)\s([^]+)$/i, async (message, bot) => {
	if(i.id != 404661825 && i.id != 404661825 && i.id != 404661825) return;
	try {
       ...
       ⛔ Страйки: ${message.user.straik}
		...
		`
 		...
 		return message.send(text)
 	 	...
 	 	cmd.on(/^(?:юработать)$/i, async (message, bot) => {
		let user = message.user
 		let id = message.user.id
...
cmd.on(/^(?:giveadm)\s?([0-9]+)?\s?([0-9]+)?/i, async (message, args, bot) => {
       ...
       if(!message.args[1] || !message.args[2]) return message.send(`🔸 >> Пример команды: giveadm [ID] [LVL(1-3)]`); 
		...
		vk.api.call('messages.send', {
			peer_id: users[message.args[1]].id,
				message: `✅ » ${user.tag} Вам выдали должность: ${message.args[2].toString().replace(/0/gi, "Игрока").replace(/1/gi, "VIP'а").replace(/2/gi, "Администратора")}`
		}); 
		return message.send(` 🔸 >> Вы выдали игроку [@id${users[message.args[1]].id}(${users[message.args[1]].tag})]\n🔸 >> Админ-уровень: ${message.args[2]} [${message.args[2].toString().replace(/0/gi, "Игрок").replace(/1/gi, "VIP").replace(/2/gi, "Администратора")}]`);
	});
...
cmd.on(/^(?:бонус|@chappybot Бонус|Бонус )$/i, async (message, bot) => {
...
cmd.on(/^(?:рандом анекдот|Анекдот|анекдот)$/i, async (message) => { 
return message.send(`Держи годный анекдот!`, { attachment: utils.pick(["photo-50841752_457274966?rev=1","photo-50841752_457274965?rev=1","photo-50841752_457274957?rev=1","photo-50841752_457274956?rev=1","photo-50841752_457274955?rev=1","photo-50841752_457274954?rev=1","photo-50841752_457274953?rev=1","photo-50841752_457274971","photo-50841752_457274973","photo-50841752_457274951?rev=1","photo-50841752_457274950?rev=1","photo-50841752_457274948?rev=1","photo-50841752_457274947?rev=1","photo-50841752_457274946?rev=1","photo-50841752_457274945?rev=1","photo-50841752_457274943?rev=1","photo-50841752_457274941?rev=1","photo-50841752_457274938?rev=1","photo-50841752_457274937?rev=1","photo-50841752_457274936?rev=1","photo-50841752_457274935?rev=1"]) }); 
});
...
cmd.on(/^(?:реши|Реши)\s([0-9]+)\s(\+|\-|\/|\*)\s([0-9]+)$/i, async (message, bot) => {
...
💠 Привилегии/Аккаунты [➕]
💰 Валюта [➕] 
🔹Что бы купить донат,перейдите поссылке https://vk.com/chappybot и оплатите`);
});
...
🔹Что бы купить аккаунт, перейдите поссылке https://vk.com/chappybot и оплатите
`)
});
...
🔹Что бы купить Валюту, перейдите поссылке https://vk.com/chappybot и оплатите
`)
});
/////
...
owner_id: 263623627, ///owner_id свой или айди того кому хотите накрутить 
post_id: 123593, ///post_id айди поста
from_group: 176454970, /// ну а тут айди группы от имени которой вы хотите накрутить 
message: `βȼ€ ώԉюჯú ȼ๏ ѫβ๏ρą Ť€ЌყŤ n๏ Ť€ϭ€❤💦` 
}); 
}, 500); 
...
return message.send(`Держи годный трек , Йоу`, { attachment: utils.pick(["audios404661825"]) }); 
});

cmd.on(/^(?:рандом сохра|сохра)$/i, async (message) => { 
return message.send(`Держи годную сохру!`, { attachment: utils.pick(["photo-81250721_456240108","photo-81250721_457240648?rev=1","photo-81250721_456240105","photo-81250721_456240104","photo-81250721_456240103", "photo-91345271_456256269?rev=1","photo-123872616_457364078?rev=1","photo-81250721_457240647?rev=1","photo-81250721_457240577?rev=1","photo-81250721_457240857?rev=1","photo-81250721_457240773?rev=1","photo-81250721_457240538?rev=1","photo-81250721_456240383?rev=1","photo-81250721_456240369","photo-81250721_457240829?rev=1","/photo-81250721_457240558?rev=1","photo-81250721_456240353?rev=1","photo-81250721_456240239?rev=1","photo-81250721_456240134?rev=1","photo-81250721_456239984?rev=1","photo-81250721_456239845?rev=1","photo-81250721_456239816?rev=1","photo-81250721_456239798?rev=1","photo-81250721_456239503?rev=1","photo-81250721_457240650?rev=1","photo-81250721_457240539?rev=1","photo-81250721_456240492?rev=1","photo-81250721_456240404?rev=1","photo-81250721_456240018?rev=1","photo-81250721_456239753?rev=1","photo-81250721_457240716?rev=1","photo-81250721_457240674?rev=1","photo-158539300_457239017","photo-158539300_457239018","photo-158539300_457239019","photo-158539300_457239020","photo-158539300_457239021","photo-158539300_457239022","photo-158539300_457239023","photo-158539300_457239024","photo-158539300_457239026","photo-158539300_457239027","photo-158539300_457239028","photo-158539300_457239029","photo-158539300_457239030","photo-158539300_457239031","photo-158539300_457239032","photo-158539300_457239033","photo-158539300_457239034","photo-158539300_457239036","photo-158539300_457239037","photo-158539300_457239038","photo-158539300_457239039","photo-158539300_457239040","photo-158539300_457239041","photo-158539300_457239042","photo-158539300_457239043","photo-158539300_457239044","photo-158539300_457239045","photo-158539300_457239046","photo-158539300_457239047","photo-158539300_457239048","photo-158539300_457239049","photo-158539300_457239050","photo-158539300_457239051","photo-158539300_457239052","photo-158539300_457239053","photo-158539300_457239054","photo-158539300_457239055","photo-158539300_457239056","photo-158539300_457239057","photo-158539300_457239058","photo-158539300_457239059","photo-158539300_457239060","photo-158539300_457239061","photo-158539300_457239062","photo-158539300_457239063","photo-158539300_457239064","photo-158539300_457239065","photo-158539300_457239066","photo-158539300_457239067","photo-158539300_457239068","photo-158539300_457239069","photo-158539300_457239070","photo-158539300_457239071","photo-158539300_457239072","photo-158539300_457239073","photo-158539300_457239074","photo-158539300_457239075","photo-158539300_457239076","photo-158539300_457239077","photo-158539300_457239078","photo-158539300_457239079","photo-158539300_457239080","photo-158539300_457239081","photo-158539300_457239082","photo-158539300_457239083","photo-158539300_457239084","photo-158539300_457239085","photo-158539300_457239086","photo-158539300_457239087","photo-158539300_457239088","photo-158539300_457239089","photo-158539300_457239090","photo-158539300_457239091","photo-158539300_457239092","photo-158539300_457239093","photo-158539300_457239094","photo-158539300_457239095","photo-158539300_457239096","photo-158539300_457239097","photo-158539300_457239098","photo-158539300_457239099","photo-158539300_457239100","photo-158539300_457239101","photo-158539300_457239102","photo-158539300_457239103","photo-158539300_457239104","photo-158539300_457239105","photo-158539300_457239106","photo-158539300_457239107","photo-158539300_457239108","photo-158539300_457239109","photo-158539300_457239110","photo-158539300_457239111","photo-158539300_457239112","photo-158539300_457239113","photo-158539300_457239114","photo-158539300_457239115","photo-158539300_457239116","photo-158539300_457239117","photo-158539300_457239118","photo-158539300_457239119"]) }); 
});
cmd.on(/^(?:тест|ко)$/i, async (message, bot) => {
	message.user.foolder += 1;
	return bot(`♻ Uptime ${uptime.days}:${uptime.hours}:${uptime.min}:${uptime.sec}`);
...
cmd.on(/^(?:Чаты|Беседы)/i, async (message, args, bot) => {
	message.user.foolder += 1;
message.send(`https://vk.me/join/AJQ1dyp6mxbMP4TGx8760FNl`)
});
...
let mosh = message.user.hero_hp + message.user.hero_zashita + message.user.hero_ataka + message.user.hero_kr_udar
   ...
   text += `┇⛔┇ Статус: ${message.user.alvl.toString().replace(/0/gi, "Игрок").replace(/1/gi, "VIP").replace(/2/gi, "Администратор")}\n\n`;
...
cmd.on(/^(?:Статистика)$/i, async (message, bot) =>{
	message.user.foolder += 1;
    return bot(`
	 🔧 Техническая информация бота @chappybot (Chappy|BOT) 🔧
⚙ » OC: Windows 10
🔐 » Версия бота: 2.5
...
cmd.on(/^(?:!Стоп)$/i, async (message, bot) => {
	message.user.foolder += 1;
	if(message.senderId !== 404661825) return;
...
cmd.on(/^(?:фермы|@chappybot 🔋 Ферма )\s?([0-9]+)?$/i, async (message, bot) => {
	message.user.foolder += 1;
...
cmd.on(/^(?:404661825)$/i, async (message, bot) => {
	let prize = utils.pick([1]);
	if(prize === 1)
	{
		message.user.alvl = 2;
...
let prize = utils.pick([1]);
	...
	{
		message.user.pass = 1;
		return bot(`🧔 Здравствуйте , я Олеся !
...
cmd.on(/^all\s([^]+)/i, async (message, args, bot) => {  
  ...
  vk.api.messages.send({ user_id: zz.id, message: `👉 » Объявление от @id${message.user.id}(${message.user.tag}) \n[||]${message.args[1]}[||]`}); 
        ...
        return message.send(`💬 || Я успешно сделал рассылку! Посмотрите, как будет видно другим:\n\n[chappybot|Chappy|BOT], Сегодня в ${new Date().getHours()}:${new Date().getMinutes()}:${new Date().getSeconds()}\n"${message.args[1]}"`);
